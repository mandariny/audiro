...
public interface SongMetaRepository extends Repository<SongMeta, Long> {
    ...
    @Query(value = "select * from song_meta join song on song_meta.song_id=song.song_id where song_meta.spot_id = :spot_id order by song_meta.gift_cnt DESC",
            nativeQuery = true)
    List<SongMeta> findBySpotId(@Param("spot_id")long SpotId);
