/*
 * 사용자가 특정 요청을 보냈을때
 * 권한 검증 전에 Jwt필터를 거치는데,
 * 1. 토큰이 있는 경우,
 * jwt토큰이 잘 통과하면 권한을 가진채로 로그인 상태를 가지고 다음 검증 단계를 통과할 수 있다.
 * 토큰이 위조되었거나 내용이 이상한경우 401코드로 "wrong token"메세지
 * 토큰이 단순히 만료된 경우에는 401코드로 "expired" 메세지를 가진 response를 받는다.
 * 프론트에서는 expired 메세지를 받은 경우에 서버에 token/refresh를 요청한다.
 *
 * 2. 토큰이 없는 경우
 * 토큰이 없는 경우에는 다음 검증 단계에서 권한이 없는 상태로 인증과정에서 로그인 페이지로 리다이렉트된다.
 * */
